...
function log.notify(scope, level, verbose, str, ...)
    if not verbose and _G.Devcontainers.config ~= nil and not _G.Devcontainers.config.debug then
        return
    end
    ...
    vim.notify(
        string.format("[devcontainers.nvim@%s] %s", scope, string.format(str, ...)),
        level,
        { title = "devcontainers.nvim" }
    )
end
...
function log.warn_deprecation(options)
    ...
    if uses_deprecated_option then
        ...
        log.notify(
            ...
            vim.log.levels.WARN,
            true,
            "use `:h Devcontainers.options` to read more."
        )
    end
end
